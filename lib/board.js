...
var Materials = require("./materials/materials.js");
...
Board.fromState = function(state){
	...
	var objects_by_position = {};
	// PolaProject to miejsce jest problematyczne, bo robi forEach na object_position, więc chce integerów w mapie
	state.object_positions.forEach(function(position, id){
		var position_hash = position_hasher(position);
		if(objects_by_position[position_hash]===undefined){
			objects_by_position[position_hash] = [];
		}
		objects_by_position[position_hash].push(state.registred_objects.get(parseInt(id)));
	});
	ret.objects_by_position = Immutable.fromJS(objects_by_position);
...
Board.random = function(size){	
	var plansza = new Board(size);

	console.log("generating walls...");
	var amount_of_walls = Math.floor(Math.random()*Math.pow(size,2)*0.5);
	for(var i=1; i<=amount_of_walls; i++){
		var field = plansza.get_random_field();
		field.set_material(Materials.rock);
	}
	...
	for(var i=1; i<=amount_of_crates; i++){
		...
		do{
			...
			if(!field.is_obstacle()){
				var skrzynka = new BoardObject(i, "crate", String.fromCharCode(9632), "yellow");
	...
	var agent = new BoardObject(0, "agent", String.fromCharCode(9632), "green", 5, 5);
	...
	if(handle_physics){
		if(this.field_has_objects(to)){
			...
			objects.forEach(function(object){
				this.move_object(object.id, direction);				
			}.bind(this))
		}		
	}
...
Board.prototype.field_is_goal = function(x, y){
	return (this.goal_position.x == x && this.goal_position.y == y);
}
...
Board.prototype.perform_agent_action = function(action_path){
	if(action_path.length>0){
		var action = action_path.pop();
		var agent_id = this.get_object_by_name("agent").id;
		this.move_object(agent_id, action);
		setTimeout(function(){
			this.perform_agent_action(action_path);
		}.bind(this), 100);
	}
}
